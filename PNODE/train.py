...
def main(_run, _config, _log):
    ...
    if _config["use_ode"]:
        model = FewShotSegOde(pretrained_path=_config['path']['init_path'], pretrained_ode=_config["pretrain_ode"], ode_layers=_config["ode_layers"], ode_time=_config["ode_time"])
    else:
    ...
    for i_iter, sample_batched in enumerate(tqdm(trainloader)):
        # Prepare input
        s_x_orig = sample_batched['s_x'].cuda()  # [B, Support, slice_num=1, 1, 224, 224]
        ...
        optimizer.zero_grad()
        query_pred = model(s_xs, s_y_fgs, s_y_bgs, q_xs) #[B, 2, w, h]
        ...
        if (i_iter + 1) % _config['print_interval'] == 0:
            ...
            print(f'step {i_iter+1}/{total_iter}: loss: {loss}')
