...
class FewShotSeg(nn.Module):
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs, factor=1, return_feats=False):
        ...
        for epi in range(batch_size):
            ...
            # dist_supports = [[
            #     self.calDist(
            #         torch.stack(supp_fg_fts[way]).view((n_shots, -1, 1, 1)), prototype
            #     ) for prototype in prototypes] for way in range(n_ways)] # n_ways x (1+n_ways) x n_shots x 1 x C
            # dist_supports = [dist_supports[i][i+1] for i in range(n_ways)] # n_ways x n_shots x 1 x C
            # dist_supports = [d[n_shots//factor:] for d in dist_supports] # n_ways x n_shots5//factor x 1 x C
            # dist_supports_batch.append(sum([sum(dist_supports[way]) for way in range(n_ways)]))

            if self.config["adversarial_train"] and self.training:
                dist_supports = [
                    self.calDist(
                        supp_fts[:, n_shots//factor:, epi].view(-1, supp_fts.shape[-3], supp_fts.shape[-2], supp_fts.shape[-1]), prototype
                    ) for prototype in prototypes] # 
                temp_mask = F.interpolate(fore_mask[:, :, epi], size=dist_supports[-1].shape[-2:], mode='bilinear')
                dist_supports_batch.append(torch.sum(dist_supports[-1] * temp_mask)/(torch.sum(temp_mask)+1e-5))
        ...
        if return_feats:
            ...
            return output, align_loss / batch_size, sum(dist_supports_batch)  / batch_size # B x shots x P x ways
