...
def fast_gradient_method(
    ...
    # Compute loss
    # y = (y>0).to(torch.float)
    # print(y.dtype)
    # print(model_fn(x).dtype)
    loss = loss_fn(model_fn(x), y)
    # print(y.sum(), model_fn(x).sum())
    # print(loss)
...
def perturb_adv(x, model, method, add_gaussian=False):
    init_shape = x.shape
    ...
    # for e in [0.02]:
        ...
        augment_factor += 1
        if add_gaussian:
            noise = p.clone().detach().normal_(mean=0, std=0.1)
            perturbed.append((p + noise).view(init_shape))
            augment_factor += 1
    perturbed = torch.cat(perturbed, dim=1)
    return perturbed, augment_factor
